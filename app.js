...
function render(){
  ...
  if(detailId)renderDetail(); renderFeatured();
}
let detailKind='',detailReverse=false;
function renderFeatured(){const s=catalog.stories.find(s=>s.season===7)||catalog.stories[0];if(s){$('#hero-art').innerHTML=cover(s);$('#featured-title').textContent=s.series||s.title;}}
...
async function playEpisode(id,fromStart=false){
  const story=catalog.stories.find(s=>s.episodes.some(e=>e.id===id)),episode=story?.episodes.find(e=>e.id===id);if(!episode)return;if(!episode.has_audio){toast('这集录音还在准备中');return}
  ...
  $('#player-title').textContent=episode.title;$('#player-subtitle').textContent=(story.series?story.series+' 第'+story.season+'季':story.title)+' · '+(episode.kind||'故事');$('#player-cover').innerHTML=cover(episode.cover_url?episode:story);$('#elapsed').textContent='00:00';$('#duration').textContent=format(episode.duration);$('#seek').value=0;
...
$('#close-detail').onclick=closeDetail;$('#detail-resume').onclick=guarded(async()=>{const s=catalog.stories.find(s=>s.id===detailId),e=resumeEpisode(s);if(e)await playEpisode(e.id)});
