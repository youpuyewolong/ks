...
function renderUploadBoard(states){
  ...
  const labels={ready:'已匹配 · 可上传',conflict:'重复占用 · 未上传',ambiguous:'多个名称候选 · 未上传',unmatched:'未匹配 · 未上传',missing:'目标已删除 · 未上传',error:'上传失败',done:'已上传',discarded:'已舍弃 · 不上传',uploading:'正在上传'};
...
$('#export-upload-issues').onclick=()=>{
  ...
  const lines=states.filter(s=>s.state!=='done').map(s=>{const q=queue[s.index];return q.file.name+'\t'+({ready:'可上传',conflict:'重复占用',ambiguous:'多个名称候选',unmatched:'未匹配',missing:'目标已删除',error:'上传失败',discarded:'已舍弃',uploading:'上传中'}[s.state]||s.state)+'\t'+(q.message||q.reason||'')});
