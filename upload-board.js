...
function renderUploadBoard(states){
  ...
  const left=episodes.filter(e=>(!uploadKind()||(e.kind||'故事')===uploadKind())&&(e.title+' '+e.subtitle).toLowerCase().includes(query)&&(!$('#board-candidates-only').checked||candidateIds.has(e.id)));
  ...
  const visible=StoryMatching.orderFiles(queue,episodes,$('#board-file-sort').value).filter(i=>{
    ...
    return (q.file.name+' '+(target?.title||'')).toLowerCase().includes(search)&&(filter==='outside'?s.state==='outside':filter==='issues'?s.issue:filter==='ready'?s.ready:filter==='discarded'?s.state==='discarded':s.state!=='discarded'&&s.state!=='outside');
  });
